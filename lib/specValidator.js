...
class SpecValidator {
  ...
  updateValidationResult(where, code, message, innerErrors) {
    ...
    this.updateValidityStatus();
    if (!this.specValidationResult[where]) {
      this.specValidationResult[where] = {};
    }
    this.specValidationResult[where] = err;
    return;
  }
  ...
  validateSpec() {
    ...
    if (!self.swaggerApi) {
      ...
      self.updateValidationResult('initialize', 'INITIALIZATION_ERROR', msg);
      throw new Error(msg);
    }
    try {
  ...
  validateXmsExamples(operation) {
    ...
    if (xmsExamples) {
      for (let scenario in xmsExamples) {
        ...
        let result = self.validateRequest(operation, xmsExample.parameters);
        log.info(`${operation.operationId}: x-ms-example request validation\n${util.inspect(result.validationResult, {depth: null})}`);
  ...
  validateExampleRequest(operation) {
    ...
    if (bodyParam && bodyParam.schema && bodyParam.schema.example) {
      ...
      let result = self.validateRequest(operation, exampleParameterValues);
