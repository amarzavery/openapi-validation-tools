...
var fs = require('fs'),
  ...
  UmlCreator = require('./validators/umlCreator');
...
exports.createUml = function createUml(specPath, outputDir, options) {
  if (!options) options = {};
  log.consoleLogLevel = options.consoleLogLevel || log.consoleLogLevel;
  log.filepath = options.logFilepath || log.filepath;
  let specFileName = path.basename(specPath);
  ...
  return utils.parseJson(specPath).then((result) => {
    ...
    let umlCreator = new UmlCreator(resolver.specInJson);
    return umlCreator.generateDiagramFromGraph();
  }).then((svgGraph) => {
    if (outputDir !== './' && !fs.existsSync(outputDir)) {
      fs.mkdirSync(outputDir);
    }
