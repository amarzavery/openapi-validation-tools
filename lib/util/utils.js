...
var fs = require('fs'),
  ...
  InheritanceTree = require('./inheritanceTree');
...
/**
 * Creates an InheritanceTree for a given model in the inheritance chain
 *
 * @param {object} specInJson - The swagger spec
 * @param {string} name - Name of the model for which the tree needs to be created.
 * @param {Map<string, InheritanceTree>} subTreeMap - A map that stores a reference to InheritanceTree for a given model in the inheritance chain.
 * @param {object} [options] - Optional parameters
 * @param {string} [options.discriminator] - Name of the property that is marked as the discriminator.
 * @returns {InheritanceTree} rootNode- A InheritanceTree that represents the model in the inheritance chain.
 */
exports.createInheritanceTree = function createInheritanceTree(specInJson, name, subTreeMap, options) {
  ...
  let rootNode = new InheritanceTree(name);
  let definitions = specInJson.definitions;
  let discriminator = options.discriminator;
  ...
  let children = exports.findChildren(specInJson, name);
  for (let childName of children) {
    let childObj = exports.createInheritanceTree(specInJson, childName, subTreeMap, options);
 ...
 */
exports.findChildren = function findChildren(specInJson, name) {
  if (name === null || name === undefined || typeof name.valueOf() !== 'string' || !name.trim().length) {
    throw new Error('name is a required property of type string and it cannot be an empty string.')
  }
  let definitions = specInJson.definitions;
